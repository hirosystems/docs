  ...
  generateMethodSection(method: TypeDocExport): string {
    ...
    if (signature.examples && signature.examples.length > 0) {
      ...
      if (signature.examples.length === 1) {
        const { language, code } = this.extractCodeFromExample(
          signature.examples[0].code
        );
    ...
    if (signature.returns) {
      ...
      sections.push(
        `- **Type:** \`${this.escapeForMarkdown(signature.returns.type)}\``
      );
    ...
    if (method.source) {
      ...
      sections.push(
        `> **Source:** [${fileName}:${method.source.line}](${method.source.url})`
      );
    }
  ...
  generateNamespaceSection(): string {
    ...
    namespaces.forEach((ns) => {
      ...
      const namespaceFunctions = this.exports.filter((e) => {
        ...
        if (e.source?.fileName.includes(`/${ns.name.toLowerCase()}.ts`))
          return true;

        // For Cl namespace, check if it's in the cl.ts file
        if (ns.name === "Cl" && e.source?.fileName.includes("/cl.ts"))
          return true;

        // For Address namespace, check if it's in the address namespace file
        if (
          ns.name === "Address" &&
          e.source?.fileName.includes("/namespaces/address.ts")
        )
          return true;
      ...
      if (namespaceFunctions.length > 0) {
        ...
        namespaceFunctions.forEach((func) => {
          ...
          sections.push(
            `    <AccordionTrigger>${ns.name}.${functionName}</AccordionTrigger>`
          );
          sections.push(`    <AccordionContent>`);
          ...
          if (signature) {
            ...
            if (signature.returns) {
              ...
              if (signature.returns.description) {
                sections.push(
                  `- **Description:** ${signature.returns.description}`
                );
              }
              sections.push("");
            }
          ...
          if (func.source) {
            ...
            sections.push(
              `> **Source:** [${fileName}:${func.source.line}](${func.source.url})`
            );
          }
    ...
    if (interfaces.length > 0) {
      ...
      interfaces.forEach((iface) => {
        const desc = iface.description
          ? this.escapeForMarkdown(iface.description)
          : "-";
    ...
    if (types.length > 0) {
      ...
      types.forEach((type) => {
        const desc = type.description
          ? this.escapeForMarkdown(type.description)
          : "-";
