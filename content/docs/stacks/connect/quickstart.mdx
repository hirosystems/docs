...
<div className='steps space-y-12'>
  ...
  <Step>
    <div className="flex flex-col space-y-3">
      <div className="w-full">
        <p className='text-lg font-bold m-0 dark:text-primary-foreground'>Interacting with the wallet</p>
        <p className='mb-0'>The process of connecting a wallet gives the web app information about a wallet account which enables interactions with the Stacks blockchain, like calling smart contracts.</p>
      ...
      <div className="w-full">
        ...
        <p className='mb-0'>This will configure the `showConnect` function to display a wallet connection pop-up with the app's name and icon, and handling user confirmation or cancellation actions in the `onFinish` and `onCancel` callbacks.</p>
      </div>
    </div>
  </Step>
  ...
  <Step>
    <div className="flex flex-col space-y-3">
      ...
      <div className="w-full">
        ...
        <Callout>
          The `amount` field is denominated in `microSTX`, for example 1_000_000 microSTX = 1 STX.
        </Callout>
