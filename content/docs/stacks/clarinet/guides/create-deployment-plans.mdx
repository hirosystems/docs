...
<Callout title="Note">
  When deploying, Clarinet prompts you to overwrite any manually added or edited
  changes that differ from the specifications laid out in your project's .toml
  files. Type `no` to proceed with your manually configured deployment plan
...
<Callout title="Note">
  Read-only functions and functions that take no arguments are passed an empty
  list like so: `parameters: []`
</Callout>
