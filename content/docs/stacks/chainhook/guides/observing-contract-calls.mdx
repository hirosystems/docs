...
View the full predicate example we will be using in this guide below.
...
Next, define the scope of the `predicate` within the `if_this` specification. The `contract_call` scope within your predicate allows Chainhook to observe blockchain data when the specified method is directly called from its contract. In this scope, you identify the contract that contains the method you want to observe using `contract_identifier` and the name of the function in `method`.
...
Finally, define how Chainhook delivers the payload when it is triggered by your `predicate` within the `then_that` specification.
...
With your `predicate` set up, we can now use Chainhook to monitor blocks that match the `contract_call` scope and analyze the returned payload. Chainhook will track events where this function is directly invoked in a transaction and generate a detailed payload containing transaction data at the block level.

Specify your `contract-call-chainhook.json` predicate file in the following command to scan the Stacks blockchain.
...
For your `contract_call`, the relevant data is housed in the `data` object of these elements. Similar to the `kind` object, the `data` object includes a `contract_identifier` key, which in this case is linked to the `simple-vote-v0` contract.
