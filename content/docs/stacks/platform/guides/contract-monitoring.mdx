...
description: Learn how to monitor your contract, post-mainnet deployment, with custom alerts.
---

## Prerequisites

- Create or import a project by following the steps in the [create project](/stacks/platform/guides/create-project) guide.
- Deploy a project's Clarity smart contract to mainnet by following the [Deployment](/stacks/platform/guides/deploy-project) guide.
...
Your contract will then appear under Contract Monitoring. Once you are monitoring a contract, you can view its transaction history and any pending transactions in the mempool. Click on "Create an alert" to create a specific alert you want to monitor for. 
...
Once you identify the calls you want to monitor, you can further hone in on the specific function calls by creating specific filters for argument values and the address of the wallet which called the function. 

The specific conditions you can choose to monitor for is when a:

- Function is called
- Function is called with argument
- Function is called by address

You can monitor these alerts in the Hiro Platform, and you can also choose to receive email alerts every time one of those alerts is triggered. Or configure webhook calls, which can be used for all kinds of customized alerting flows.
...
We hope this new feature increases your confidence in your code as well as your code’s security.
