...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/burnchain/rewards?limit=96&offset=42000"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/burnchain/rewards?limit=96&offset=42000", {
  method: "GET"
});
```
