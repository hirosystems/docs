...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/contract/by_trait?trait_abi=string&limit=0&offset=0"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/contract/by_trait?trait_abi=string&limit=0&offset=0", {
  method: "GET"
});
```
