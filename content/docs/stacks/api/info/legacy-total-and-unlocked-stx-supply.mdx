...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/stx_supply/legacy_format?height=200"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/stx_supply/legacy_format?height=200", {
  method: "GET"
});
```
