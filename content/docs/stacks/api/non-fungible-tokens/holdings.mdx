...
<Property required={false} deprecated={false} name={"asset_identifiers"} type={"array of string"}>
...
<Property required={false} deprecated={false} name={"tx_metadata"} type={"boolean"}>
...
```bash title="Terminal"
curl -X GET "https://api.mainnet.hiro.so/extended/v1/tokens/nft/holdings"
```
