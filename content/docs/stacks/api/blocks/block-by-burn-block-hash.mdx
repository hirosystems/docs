...
<APIPlayground authorization={undefined} method={"GET"} route={"/extended/v1/block/by_burn_block_hash/{burn_block_hash}"} path={[]} query={[]} header={[]} body={undefined} schemas={{}}>
...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/block/by_burn_block_hash/%7Bburn_block_hash%7D"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/block/by_burn_block_hash/%7Bburn_block_hash%7D", {
  method: "GET"
});
```
