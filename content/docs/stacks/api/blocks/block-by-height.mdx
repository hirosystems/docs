...
This endpoint is deprecated in favor of [Get blocks](/stacks/api/blocks/get-block).
</Callout>
...
<APIPlayground authorization={undefined} method={"GET"} route={"/extended/v1/block/by_height/{height}"} path={[]} query={[]} header={[]} body={undefined} schemas={{}}>
...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/block/by_height/%7Bheight%7D"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/block/by_height/%7Bheight%7D", {
  method: "GET"
});
```
