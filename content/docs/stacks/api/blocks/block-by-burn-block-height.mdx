...
<APIPlayground authorization={undefined} method={"GET"} route={"/extended/v1/block/by_burn_block_height/{burn_block_height}"} path={[]} query={[]} header={[]} body={undefined} schemas={{}}>
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/block/by_burn_block_height/%7Bburn_block_height%7D", {
  method: "GET"
});
```
