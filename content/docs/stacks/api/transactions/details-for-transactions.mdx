...
<APIInfo method={"GET"} route={"/extended/v1/tx/multiple"}>
...
<APIPlayground authorization={undefined} method={"GET"} route={"/extended/v1/tx/multiple"} path={[]} query={[]} header={[]} body={undefined} schemas={{}}>
...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/tx/multiple"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/tx/multiple", {
  method: "GET"
});
```
...
export interface TransactionList {
  [k: string]: TransactionFound | TransactionNotFound;
}
/**
