...
<APIInfo method={"GET"} route={"/extended/v2/blocks/{height_or_hash}/transactions"}>
...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v2/blocks/42000/transactions"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v2/blocks/42000/transactions", {
  method: "GET"
});
```
