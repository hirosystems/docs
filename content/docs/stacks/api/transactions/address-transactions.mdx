...
<h2 className="not-prose mb-2 inline-flex items-center gap-3 font-mono">
  ...
  <p className="text-xs break-words overflow-hidden w-[95%]">{`/extended/v2/addresses/{address}/transactions`}</p>
</h2>
...
```bash title="curl"
curl -X GET "https://api.mainnet.hiro.so/extended/v2/addresses/SP31DA6FTSJX2WGTZ69SFY11BH51NZMB0ZW97B5P0/transactions"
```
