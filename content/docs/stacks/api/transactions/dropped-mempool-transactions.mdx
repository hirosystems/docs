...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v1/tx/mempool/dropped?limit=96&offset=42000"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v1/tx/mempool/dropped?limit=96&offset=42000", {
  method: "GET"
});
```
