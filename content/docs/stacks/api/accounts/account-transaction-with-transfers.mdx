...
<h2 className="not-prose mb-2 inline-flex items-center gap-3 font-mono">
  ...
  <p className="text-xs">{`/extended/v1/address/{principal}/{tx_id}/with_transfers`}</p>
</h2>

Retrieves transaction details for a given `tx_id`, for a given account or contract Identifier.
