...
```bash
curl -X GET "https://api.mainnet.hiro.so/extended/v2/pox/cycles?limit=20&offset=20"
```
...
```js
fetch("https://api.mainnet.hiro.so/extended/v2/pox/cycles?limit=20&offset=20", {
  method: "GET"
});
```
