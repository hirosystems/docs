---
title: bit-not
description: Using the bit-not function for bitwise complement operations in Clarity smart contracts.
---
...
(define-data-var user-flags uint u0)

(define-public (toggle-flag (flag uint))
  (ok (var-set user-flags (bit-xor (var-get user-flags) flag))))

(define-public (clear-all-flags-except (flag uint))
  (ok (var-set user-flags (bit-and (var-get user-flags) flag))))

(define-public (set-all-flags-except (flag uint))
  (ok (var-set user-flags (bit-and (bit-not (var-get user-flags)) flag))))

(define-read-only (has-flag (flag uint))
  (is-eq flag (bit-and (var-get user-flags) flag)))
```
