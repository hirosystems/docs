...
```clarity
(define-map asset-prices uint uint)

(define-public (set-price (price uint))
  (ok (map-set asset-prices block-height price)))

(define-read-only (was-price-above-at-block (threshold uint) (block-hash (buff 32)))
  (at-block block-hash
    (let ((past-price (default-to u0 (map-get? asset-prices block-height))))
      (> past-price threshold))))
