...
(define-public (register-user)
  (begin
    (asserts! (is-none (map-get? users tx-sender)) (err u1)) ;; Check if user already exists
    (map-set users tx-sender true)                           ;; Register the user
    (var-set user-count (+ (var-get user-count) u1))         ;; Increment user count
    (print (concat "New user registered: " (to-ascii (serialize tx-sender))))
