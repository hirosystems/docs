...
(define-public (transfer (amount uint) (recipient principal))
  (let ((sender-balance (default-to u0 (map-get? balances tx-sender))))
    ...
    (asserts! (is-eq tx-sender recipient false) (err u2)) ;; Can't send to self
...
;; Usage
(transfer u100 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM) ;; Returns (ok true) if successful
```
