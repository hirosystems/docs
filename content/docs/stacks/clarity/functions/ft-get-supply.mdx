...
(define-public (mint-if-under-cap (amount uint) (recipient principal))
  (let ((current-supply (ft-get-supply my-token)))
    (if (<= (+ current-supply amount) max-supply)
        (ft-mint? my-token amount recipient)
        (err u1))))

;; Usage
(mint-if-under-cap u500000 tx-sender) ;; Returns (ok true)
(ft-get-supply my-token) ;; Returns u500000
(mint-if-under-cap u600000 tx-sender) ;; Returns (err u1)
```
