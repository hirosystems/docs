...
Check out the [API Reference](/stacks/stacks.js/packages/transactions) page for `@stacks/transactions` to learn more about building different types of transactions.
...
1. A running instance of the `devnet` network. This can be run using `clarinet devnet start`.
  ...
  <Step>
    <div className="flex flex-col space-y-3">
      <div className="w-full">
        ...
        <p>To set up a STX token transfer transaction, use the `makeSTXTokenTransfer` function. This function requires a `senderKey`, `recipient` address, `amount` to be sent in uSTX, a `network` instance, and an `anchorMode`.</p>
      </div>
      ...
      <p className='mb-0'>Optional fields for a STX transfer transaction include `memo`, `nonce`, and `fee`:.</p>
  ...
  <Step>
    <div className="flex flex-col space-y-3">
      ...
      <div className="w-full">
        ...
        <p className='mb-0'>On success, this will return a `StacksTransaction` object that contains information about the transaction, including the `txid`.</p>
      </div>
    </div>
  </Step>
</div>
