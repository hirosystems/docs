...
description: This guide explains how to connect to users' wallets and authenticate users with the @stacks/connect package from Stacks.js.
---
...
- Integrate user authentication in your web app using `@stacks/connect`
- Initiate a `userSession` with specific permission scopes
- Trigger the authentication flow with the `showConnect` function
- Manage pending authentication states, user data and user sign-in status
...
<div className='steps space-y-10'>
  <Step>
    <div className="flex flex-col">
      <div className="w-full">
        <p className='text-lg font-bold m-0'>Install <code className='text-lg'>@stacks/connect</code></p>
        <p>First thing we need to do is install the `@stacks/connect` package in our project.</p>
      </div>
      <div className="w-full">
        ```package-install
        @stacks/connect
  ...
  <Step>
    <div className="flex flex-col">
      <div className="w-full">
        <p className='text-lg font-bold m-0'>Initiate a <code className='text-lg'>userSession</code> with specific permission scopes</p>
        <p>After installing the `@stacks/connect` package, the next step is to initiate a `userSession` with specific permission scopes.</p>
      </div>
      <div className="w-full">
        ```ts twoslash
        ...
        <Callout title="Note">We recommend you initiate the `userSession` object just once in your app then reference it using imports where needed.</Callout>
      </div>
    </div>
  </Step>
  ...
  <Step>
    <div className="flex flex-col space-y-3">
      <div className="w-full">
        <p className='text-lg font-bold m-0'>Trigger the authentication flow with the <code className='text-lg'>showConnect</code> function</p>
        <p>Let's create an `authenticate` function that will call `showConnect`, triggering the popup that initiates the authentication process for users, one in which they'll authenticate with a [Secret Key](../concepts/private-keys) that's used to encrypt their private data.</p>
      ...
      <div className="w-full">
        ...
        <TypeTable
          type={{
            appDetails: {
              description: 'Details of the app requesting authentication',
              type: '{ name: string; icon: string; }',
              default: null,
            },
            redirectTo: {
              description: 'URL to redirect the user to upon successful authentication',
              type: 'string',
              default: null,
            },
            onFinish: {
              description: 'Callback function that executes after successful authentication',
              type: 'function',
              default: null,
            },
            userSession: {
              description: 'The userSession object initiated for authentication',
              type: 'UserSession',
              default: null,
            }
          }}
        ...
        <Accordion title="View Full Code">
          ```tsx twoslash
          // @noErrors
          "use client";
          ...
          export const App: React.FC = () => {
            ...
            return (
              <button
                className="bg-white hover:bg-gray-200 px-5 py-2 text-sm leading-5 rounded-full font-semibold text-neutral-900"
  ...
  <Step>
    <div className="flex flex-col">
      <div className="w-full">
        <p className='text-lg font-bold m-0'>Handle Pending Authentication</p>
        <p>After the authentication process is initiated, it's crucial to handle cases where the user hasn't completed the authentication flow. This involves managing the pending authentication state when the user is redirected back to the app without having confirmed their authentication in the popup window.</p>
      </div>
      <div className="w-full">
        ```tsx twoslash
        import { AppConfig, UserSession, showConnect } from '@stacks/connect';
...
### <span className='font-light'>Next step: </span> <span className='inline-flex items-center'>Create and broadcast transactions <ChevronsRight className='ml-1' /></span>

Once your users are connected to your app, you will need a way to prompt them to sign transactions and broadcast them to the Stacks blockchain. [This step-by-step guide](./broadcast-transactions) has you covered.
