...
Start by creating a new project with the Clarinet CLI. This command create a project structure inside of `defi` with the necessary files and folders, including the Clarinet JS SDK already set up for testing.
...
Since the smart contract code is out of scope for this guide, we are going to use a pre-existing contract. First, generate a new file using the `clarinet contract new` command in order setup our project with the necessary configuration and test files.
...
This function allows users to deposit STX into the contract, updating their balance inside a `deposits` map and adds to the total deposits stored in a `total-deposits` variable. The key tests we want to cover are that the deposit is successful and that the user's balance, as well as the contract's total deposits, are updated correctly.

Inside of your `defi.test.ts` file, add the following setup code:
...
This structure comes from our Vitest integration and organizes our tests and describes what we're testing. The `describe` block groups multiple test cases together, while the `it` block represents a single test case.
...
```typescript
const amount = 1000;
const depositCall = simnet.callPublicFn('defi', 'deposit', [Cl.uint(amount)], wallet1.address);
```
...
After making the deposit, verify that the call itself was successful:

```typescript
expect(depositCall.result).toBeOk(Cl.bool(true));
```

Let's go over a few of the keywords in this line:
...
```typescript
const balance = simnet.callReadOnlyFn('defi', 'get-balance-by-sender', [], wallet1.address);
expect(balance.result).toBeOk(Cl.some(Cl.uint(depositAmount)));
```
...
```typescript
  ...
  describe('stx-defi', () => {
    it('allows users to deposit STX', () => {
      const amount = 1000;
      const deposit = simnet.callPublicFn('defi', 'deposit', [Cl.uint(amount)], wallet1.address);

      const totalDeposits = simnet.getDataVar('defi', 'total-deposits');
      expect(totalDeposits).toBeUint(1000);

      const balance = simnet.callReadOnlyFn('defi', 'get-balance-by-sender', [], wallet1.address);
      expect(balance.result).toBeOk(Cl.some(Cl.uint(amount)));
    });
  });
```
  </Accordion>
