---
title: Rate Limiting
description: Rate Limiting for Hiro APIs
---

The following rate-limits (in requests per minute, or RPM) are applicable for all Hiro APIs as of [November, 2023](https://www.hiro.so/blog/updated-rate-limits-for-hiro-apis):

## API Rate Limits

| API key | Rate Limit                        |
|:--------|:----------------------------------|
| No      | 50 requests per minute (RPM) per client IP, for unauthenticated traffic |
| Yes     | 500 requests per minute (RPM), with an authenticated API key, regardless of origin IP |

These rate-limits apply across our 3 public-facing APIs:
- [Stacks Blockchain API](./api)
- [Token Metadata API](./token-metadata-api)
- [Ordinals API](../bitcoin/api)

The entirety of API traffic, regardless of whether it's from one or all of these APIs, will be attributed to a single default quota: 50 RPM for unauthenticated and 500 RPM for authenticated traffic.

These new rate limits help us ensure fair usage of Hiro APIs and provide the best possible service as our community of developers and applications grow.

<Callout title='Generate an API key'>
You can create one for free in the [Hiro Platform](https://platform.hiro.so).
</Callout>

## STX Faucet Rate Limits

| Type                        | Rate Limit                  |
|:-----------------------------|:-----------------------------|
| Stacking requests           | 1 request per 2 days        |
| Regular STX requests | 1 request per minute (RPM)     |