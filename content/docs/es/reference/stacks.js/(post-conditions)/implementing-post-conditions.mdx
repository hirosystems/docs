...
* Configurar modos de post-condición para la seguridad de transacciones
* Implementar post-condiciones para STX, tokens fungibles y NFTs
* Maneja tokens semi-fungibles (SFTs) con post-condiciones
:::
...
El ayudante Pc en Stacks.js proporciona una API fluida inspirada en BDD para construir post-condiciones. Comienza con `Pc.principal()` para especificar qué dirección será verificada, luego encadena métodos para definir la condición.
...
El ayudante de Pc utiliza encadenamiento de métodos para una construcción intuitiva de condiciones. Tu IDE proporcionará autocompletado para los métodos disponibles en cada paso.
...
* `.willSendEq(amount)` - Exactamente igual a la cantidad
* `.willSendGte(amount)` - Mayor o igual que cantidad
* `.willSendGt(amount)` - Mayor que la cantidad
* `.willSendLte(amount)` - Menor o igual que la cantidad
* `.willSendLt(amount)` - Menos que cantidad

### Métodos de tipo de activo
...
## Estableciendo el modo de post-condición

El modo de post-condición determina cómo la blockchain de Stacks maneja las transferencias de activos no cubiertas explícitamente por tus post-condiciones. Esta es una configuración de seguridad crítica.
...
* **`PostConditionMode.Deny`** (predeterminado): La transacción falla si ocurre alguna transferencia no especificada
...
Protege las transferencias de STX especificando cantidades exactas o rangos.
...
Asegúrese de que los tokens fungibles se transfieran según lo esperado en las llamadas al contrato.
...
Controla los cambios de propiedad de NFT con post-condiciones específicas.
...
Las transacciones complejas a menudo requieren múltiples post-condiciones para proteger completamente todas las transferencias de activos.
...
:::next-steps
* [Llamadas a contratos](/reference/stacks.js/contract-calls): Aprende cómo llamar a contratos en Stacks
