...
La API de Ordinals proporciona acceso completo a datos de inscripciones, artefactos digitales y seguimiento de satoshis. Todos los endpoints devuelven JSON y admiten paginación para conjuntos de resultados grandes.

URL base: `http://localhost:3000/ordinals/v1`

## Autenticación

La API está abierta por defecto. Para implementaciones en producción, configure las claves de API:
...
Incluya la clave de API en las solicitudes:
...
| `to_block` | número | Altura del bloque final |
...
### Listar rarezas de satoshi \[#list-rarities]
...
| `epic` | Primera sat después del halving |
