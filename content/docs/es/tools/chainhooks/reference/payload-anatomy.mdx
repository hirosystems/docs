...
description: Comprender la estructura de las cargas útiles de webhook de Chainhooks
---
## Resumen de Payload

Un payload de Chainhooks consta de dos secciones principales: **evento** y **chainhook**I see that you've provided the critical rules for translation, but the actual text to translate appears to be just a single colon ":".&#x20;

Since there's no substantive text content to translate, I'll return:
...
El `apply` el arreglo contiene bloques que se están agregando a la cadena canónica. Cada bloque incluye:
...
:::callout
Chainhooks maneja automáticamente las reorganizaciones enviando eventos de reversión. Su aplicación debe revertir cualquier cambio de estado de los bloques revertidos.
:::
...
:::callout
El `args` array incluye `repr` y `type` campos cuando `decode_clarity_values` está habilitado en las opciones.
:::
