...
description: Clarinet proporciona herramientas poderosas para validar, analizar y depurar tus contratos inteligentes. Desde la comprobación estática de tipos hasta el análisis de costos en tiempo real, puedes asegurarte de que tus contratos sean correctos y eficientes antes de su implementación.
---
...
| Análisis Estático             | Depuración en Tiempo de Ejecución |
|-----------------------------|-------------------------------------|
...
| Violaciones de cumplimiento de traits | Cambios de estado y efectos secundarios |
...
El `clarinet check` el comando realiza una validación exhaustiva de tus contratos sin ejecutarlos:
...
El clarinete valida múltiples aspectos de tus contratos:
...
| **Visibilidad de funciones** | Modificadores público/privado/solo lectura |

**Comprobando contratos específicos**
...
Automatice la validación en su pipeline de integración continua. Por ejemplo, use GitHub Actions:
...
La consola de Clarinet proporciona potentes herramientas de análisis en tiempo de ejecución que te permiten inspeccionar el comportamiento del contrato durante la ejecución.
...
### **Rastreo de ejecución con `::trace`**

Rastrea las llamadas a funciones para entender el flujo de ejecución:
...
Comandos de depuración de navegación:

* `step` o `s` - Entrar en sub-expresiones
* `finish` o `f` - Completar la expresión actual
* `next` o `n` - Pasar por encima de las sub-expresiones
...
### **Identificación y optimización de operaciones costosas utilizando `::trace`**
...
* Operaciones anidadas de map/filter
...
### **Analizando transacciones fallidas utilizando `::trace`**
...
* [SDK de Clarinete](/tools/clarinet/sdk-introduction): Aprende cómo usar el SDK de Clarinet.
:::
