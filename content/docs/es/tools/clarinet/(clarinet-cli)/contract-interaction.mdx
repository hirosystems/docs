...
El `clarinet console` el comando inicia una sesión interactiva con todos tus contratos desplegados en una blockchain local simulada:
...
Una vez en la consola, tienes acceso a un REPL de Clarity con los contratos de tu proyecto ya desplegados y listos para interactuar. La consola admite varias opciones potentes para diferentes escenarios de desarrollo:
...
Antes de usar datos remotos, asegúrate de agregar el contrato remoto a tu `Clarinet.toml` archivo con `clarinet requirements add SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token`.
:::

### Uso de la clave API de Hiro

Para evitar límites de tasa al usar datos remotos, establece el `HIRO_API_KEY` variable de entorno. Clarinet incluye automáticamente esta clave en todas las solicitudes de API:
...
La clave de API se envía como el `x-api-key` encabezado en todas las solicitudes a las APIs de Hiro. Puedes obtener una clave API gratuita de la [Plataforma Hiro](https://platform.hiro.so).
...
La consola proporciona varias carteras de prueba que puedes usar. Cambia entre ellas para probar escenarios multi-usuario:
...
### Trabajando con alturas de bloque
  ...
  <AccordionItem value="contrato-no-encontrado">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        Si ves errores de "uso de contrato no resuelto":
        ...
        * Uso de prefijo de contrato incorrecto (`.` para contratos locales)

        **Solución**: Verifique los contratos implementados con `::get_contracts` y use el nombre correcto.
      </div>
    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="errores-de-datos-remotos">
    <AccordionTrigger>Problemas de conexión de datos remotos</AccordionTrigger>
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        ...
        **Soluciones:**
        ...
        1. Configura tu clave API de Hiro: `export HIRO_API_KEY=your_key_here`
        2. Utilice un punto final de API personalizado: `--remote-data-api-url https://your-node.com`
