...
description: Clarinet proporciona un entorno blockchain local completo, ofreciendo todos los servicios que necesitas para el desarrollo de contratos inteligentes sin necesidad de desplegar en una red pública.
---
...
| Opción | Descripción |
|--------|-------------|
| `--manifest-path <path>` | Usar ubicación alternativa de Clarinet.toml |
| `--no-dashboard` | Mostrar registros en lugar de interfaz de usuario interactiva |
| `--deployment-plan-path <path>` | Utilizar plan de despliegue específico |
...
| `--package <path>` | Utilice la configuración preempaquetada de devnet |

:::callout
### Requisitos previos

Devnet requiere que Docker esté en ejecución. Si ves "clarinet was unable to create network", asegúrate de que Docker Desktop esté funcionando o que el demonio de Docker esté iniciado.
:::
...
Usa el `--no-dashboard` bandera para entornos de CI/CD o cuando prefieres transmitir registros en tiempo real.
...
| **API de Stacks** | 3999 | API REST para consultar datos de blockchain |
| **Base de datos Postgres** | 5432 | Indexa datos de blockchain para consultas rápidas |
| **Explorador de Stacks** | 8000 | Interfaz web para navegar transacciones |
| **Explorador de Bitcoin** | 8001 | Interfaz web para la cadena regtest de Bitcoin |

Devnet incluye cuentas preconfiguradas con saldos de STX:
...
Cuando devnet se inicia, automáticamente despliega los contratos de tu proyecto:
...
El comportamiento de Devnet se controla a través de archivos de configuración en tu proyecto.
...
Personaliza los puertos para evitar conflictos:
...
Accede a varios servicios para interactuar con la blockchain, incluyendo navegación de datos, consulta de APIs y envío de transacciones directamente.
...
<Tabs defaultValue="explorer" className="w-full">
  ...
  <TabsContent value="explorador" className="mt-4">
    ...
    * Código fuente del contrato y estado
    * Saldos de cuenta
  </TabsContent>
  ...
  <TabsContent value="rpc" className="mt-4">
    ...
    * `/v2/contracts/call-read` - Llamar a funciones de solo lectura
...
<Accordion type="single" collapsible>
  <AccordionItem value="conexión-docker">
    <AccordionTrigger>Errores de conexión de Docker</AccordionTrigger>
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        ...
        **Soluciones**:
        ...
        2. Iniciar el demonio de Docker: `sudo systemctl start docker` (Linux)
        3. Verificar permisos de Docker: `docker ps`
  ...
  <AccordionItem value="conflictos-de-puertos">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        ...
        **O configura diferentes puertos** en `settings/Devnet.toml`:
  ...
  <AccordionItem value="límites-de-recursos">
  ...
  <AccordionItem value="red-existe">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        **Error**: "Docker respondió con el código de estado 409: red con nombre `<my-project>.devnet` ya existe"

        Esto ocurre cuando una sesión anterior de devnet no se limpió correctamente, dejando atrás una red de Docker.
        ...
        * Comprueba si hay redes huérfanas después de fallos:

        ```terminal
        $ docker network prune
        ```

        **Nota**: El nombre de la red se basa en el nombre del directorio de tu proyecto, así que un `clarinet new hello-world` el proyecto tendrá un nombre de red de `hello-world.devnet`.
      </div>
    </AccordionContent>
  </AccordionItem>

  <AccordionItem value="error-de-flujo-docker">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        **Error**: "Fatal: no se puede crear la imagen: error de flujo de Docker"
        ...
        Este error suele ocurrir cuando las imágenes de Docker están dañadas o cuando los exploradores no se inician correctamente.

        **Solución 1 - Deshabilitar exploradores**:
  ...
  <AccordionItem value="fallos-de-implementación">
    <AccordionTrigger>Fallos en el despliegue de contratos</AccordionTrigger>

    <AccordionContent>
      <div className="pb-4 pt-2">
        **Verificar el orden de implementación** en `Clarinet.toml`:
