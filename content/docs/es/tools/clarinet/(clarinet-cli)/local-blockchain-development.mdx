...
description: Clarinet proporciona un entorno completo de blockchain local, ofreciendo todos los servicios necesarios para el desarrollo de contratos inteligentes sin necesidad de implementarlos en una red pública.
---
## Iniciando tu blockchain local

Para lanzar devnet con todos los servicios requeridos, ejecute el siguiente comando:
...
| `--package <path>` | Usar configuración preempaquetada de devnet |

:::callout
### Requisitos previos

Devnet requiere que Docker esté en ejecución. Si ve "clarinet was unable to create network", asegúrese de que Docker Desktop esté funcionando o que el demonio de Docker se haya iniciado.
:::
...
Utilice el `--no-dashboard` bandera para entornos de CI/CD o cuando prefieres transmitir registros en tiempo real.
...
Devnet incluye cuentas preexistentes con saldos de STX:
...
Accede a varios servicios para interactuar con la blockchain, incluyendo navegar datos, consultar APIs y enviar transacciones directamente.
...
<Accordion type="single" collapsible>
  <AccordionItem value="conexión-docker">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        **Error**: "el clarinete no pudo crear la red"
  ...
  <AccordionItem value="red-existe">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        ...
        * Usar `Ctrl+C` detener elegantemente devnet
        * Comprobar si hay redes huérfanas después de fallos:

        ```terminal
        $ docker network prune
        ```

        **Nota**: El nombre de la red se basa en el nombre del directorio de tu proyecto, por lo que un `clarinet new hello-world` el proyecto tendrá un nombre de red de `hello-world.devnet`.
      </div>
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="fallos de implementación">
    <AccordionTrigger>Fallos en la implementación de contratos</AccordionTrigger>
...
* [Implementación](/tools/clarinet/deployment): Aprende cómo desplegar tu proyecto de Clarinet.
:::
