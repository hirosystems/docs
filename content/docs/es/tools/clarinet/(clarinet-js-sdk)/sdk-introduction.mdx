...
Comienza creando un nuevo proyecto de Node.js o utilizando uno existente:

```package-install
$ init -y
```

Instala el SDK de Clarinet JS y las dependencias requeridas:
...
Crea la siguiente estructura para tus pruebas de contrato inteligente:
...
El `simnet` object está automáticamente disponible en tus pruebas y proporciona un entorno simulado de la cadena de bloques de Stacks.

## Opciones de configuración

### Configuración del clarinete

Crea un `Clarinet.toml` archivo para definir tus contratos:
...
Cuando se prueban funciones que no modifican el estado, utiliza `callReadOnlyFn`:
...
Pasa parámetros utilizando los ayudantes de tipo de Clarity apropiados:
...
Asegúrate de que tu contrato esté correctamente desplegado antes de probar sus funciones:
...
### Probando con múltiples cuentas

Simula interacciones entre diferentes usuarios:
...
Especifica la ubicación de Clarinet.toml en tu configuración de Vitest:
