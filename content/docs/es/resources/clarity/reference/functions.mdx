...
`xor` devuelve true si exactamente uno de los dos argumentos es verdadero.
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `v1`, `v2`, ... | cualquiera | Valores a comparar para igualdad |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `v1`, `v2`, ... | cualquiera | Valores para incluir en la lista |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `sequence1` | secuencia | Primera secuencia |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `sequence` | secuencia | Secuencia a medir |
...
### int-a-ascii \[#int-to-ascii]

`int-to-ascii` convierte un entero a su representación de cadena ASCII.
...
### int-a-utf8 \[#int-to-utf8]

`int-to-utf8` convierte un entero a su representación de cadena UTF-8.
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `buffer` | `buff` | Buffer a convertir |
...
| `list1`, `list2`, ... | `list` | Listas para recorrer |
...
|--------|------|-------------|
| `map-name` | símbolo | Nombre del mapa |
| `key` | cualquier | Tecla para borrar |
...
### definir-privada \[#definir-privada]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `response` | respuesta | Respuesta a probar |
...
| `thrown-value` | cualquier | Valor si ninguno/error |
...
### ¡desenvolver-err! \[#desenvolver-err]
...
`unwrap-err-panic` extrae un valor de error o provoca un pánico en tiempo de ejecución.
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `default` | cualquier | Valor predeterminado |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `expr1`, `expr2`, ... | cualquiera | Expresiones a ejecutar |
...
## Tipos opcionales y tipos de respuesta \[#optional-response-types]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | Token a acuñar |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | Token para quemar |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | Token a transferir |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | Token a consultar |
...
### define-token-no-fungible \[#define-token-no-fungible]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | NFT para acuñar |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | NFT para quemar |
...
### ¿transferencia-nft? \[#transferencia-nft]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | NFT a transferir |
| `identifier` | cualquier | identificador de NFT |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `token` | nombre-del-token | NFT a consultar |
| `identifier` | cualquier | identificador de NFT |
...
### ¿transferencia-stx? \[#transferencia-stx]
...
`stx-transfer-memo?` transfiere STX con una nota.
...
|--------|------|-------------|
| `amount` | `uint` | Cantidad para quemar |
| `sender` | `principal` | Quemando direcciones STX |
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `data` | secuencia | Datos a hashear |
...
### obtener-información-del-bloque? \[#obtener-información-del-bloque]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `property` | NombreDePropiedad | Propiedad a recuperar |
...
### obtener-información-bloque-quemado? \[#obtener-información-bloque-quemado]
...
### obtener-información-bloque-stacks? \[#obtener-información-bloque-stacks]

`get-stacks-block-info?` recupera información de bloques de Stacks.
...
### ¿obtener-información-de-permanencia? \[#obtener-información-de-permanencia]

`get-tenure-info?` recupera información sobre la permanencia.
...
|--------|------|-------------|
| `property` | NombreDePropiedad | Propiedad a recuperar |
| `block` | `uint` | Bloque de permanencia |
...
### ¿destrucción-del-principal? \[#destruccion-del-principal]
...
| Nombre | Tipo | Descripción |
|--------|------|-------------|
| `trait-ref` | rasgo | Referencia del rasgo |
...
* Solo Mainnet `SP` y `SM` las direcciones pueden gastar
* Solo testnet `ST` y `SN` las direcciones pueden gastar
...
`impl-trait` declara que un contrato implementa un trait.
...
### ¿a-consenso-buff? \[#a-consenso-buff]
...
### ¿del-consenso-buff? \[#del-consenso-buff]
...
`bit-or` realiza la operación OR bit a bit.
...
### desplazamiento-de-bits-a-la-izquierda \[#desplazamiento-de-bits-a-la-izquierda]
...
### desplazamiento-de-bits-a-la-derecha \[#desplazamiento-de-bits-a-la-derecha]
