...
Después de descargar, verifique la integridad del archivo (disponible solo para archivos de blockchain):
...
SHA256 checksum files are currently only available for **archivos de blockchain**Los archivos de API no tienen archivos de suma de verificación para su verificación.
:::

1. **Descargue el archivo de suma de verificación (solo archivos de blockchain):**
   ```terminal
...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="método-de-descarga">
    ...
    <AccordionContent>
      Para obtener las velocidades más rápidas, use `gcloud storage cp` (requiere autenticación de Google). Para simplificar, use `wget -c`Para obtener fiabilidad sin autenticación, prueba un gestor de descargas como JDownloader.
    </AccordionContent>
  </AccordionItem>
