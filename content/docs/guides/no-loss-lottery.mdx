...
description: This guide explains how to create a no-loss lottery pool that leverages stacking yield.
---

import { Code, Terminal } from 'lucide-react';
import { SmallCard } from '@/components/card';

A no-loss lottery contract offers a unique way for participants to stack their assets and potentially earn rewards without the risk of losing their initial deposit.
...
First, you will define some constants and data variables to manage the state of your contract. Constants are used for fixed values, and data variables store the state that can change during the contract execution.
...
Next, you will define an NFT to represent a ticket and a map to store participant information.
...
You will now implement the core function where participants can enter the lottery by depositing their assets.
...
Now, you will implement the function to select a winner randomly from the participants.
...
Lastly, you will implement the function to claim and distribute rewards to the winners.
...
To bootstrap CityCoins contracts, follow these steps:
