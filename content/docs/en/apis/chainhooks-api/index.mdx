...
The Chainhooks API is a REST API that allows you to programmatically manage chainhooks, configure event filters, and control webhook delivery. It provides the same functionality as the [Chainhook SDK](/tools/chainhooks/chainhook-sdk) through direct HTTP requests.

Use the Chainhooks API when:
...
For TypeScript/JavaScript projects, we recommend using the [Chainhook SDK](/tools/chainhooks/chainhook-sdk) for better developer experience.

## Next Steps

:::next-steps
* [Usage Guide](/apis/chainhooks-api/usage): Authentication and best practices
* [API Reference](/apis/chainhooks-api/reference): Complete endpoint documentation
:::
