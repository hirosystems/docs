...
description: Complete reference for all Chainhook configuration options
---
...
:::callout{type="info"}
**Defaults**: All boolean options default to `false` if omitted. Integer options are optional.
:::
...
For production with essential data only:
...
### Time-Limited Chainhook

For a chainhook that auto-expires after 1000 blocks or 100 matches:
...
```json
{
  "options": {
    "decode_clarity_values": true,
    "enable_on_registration": true,
    "expire_after_evaluations": 1000,
    "expire_after_occurrences": 100
  }
}
```

The chainhook will expire when **either** condition is met.
...
:::next-steps
* [Filter Reference](/tools/chainhook/reference/filters): Learn about all available event filters
* [Payload Anatomy](/tools/chainhook/reference/payload-anatomy): Understand the structure of chainhook payloads
:::
