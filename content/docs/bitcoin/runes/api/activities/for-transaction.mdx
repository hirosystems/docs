...
<APIInfo method={"GET"} route={"/runes/v1/transactions/{tx_id}/activity"}>
...
<Property name={"tx_id"} type={"Transaction ID"} required={true} deprecated={false}>

A transaction ID

<span>Pattern: `"^[a-fA-F0-9]{64}$"`</span>
...
```js
fetch("https://api.hiro.so/runes/v1/transactions/string/activity?offset=0&limit=1", {
  method: "GET"
});
```
