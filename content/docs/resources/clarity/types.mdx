...
Clarity provides functions for safe type conversions:

```clarity
;; Convert between integers
(to-int u100)    ;; uint to int
(to-uint 100)    ;; int to uint
...
;; Buffer conversions
(buff-to-uint-be 0x0100)   ;; Big-endian: 256
(buff-to-uint-le 0x0100)   ;; Little-endian: 1
```