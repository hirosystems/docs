...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="mainnet-simulation-addresses">
    <AccordionTrigger>Why does is-standard fail with mainnet addresses during mainnet simulation?</AccordionTrigger>
    <AccordionContent>
      <div className="pb-4 pt-2">
        When using mainnet execution simulation, the `is-standard` function expects mainnet addresses, not testnet addresses. Here's how to handle this:

        **Use mainnet addresses instead of default testnet addresses:**
        ```typescript
