...
<Accordion type="single" collapsible>
  ...
  <AccordionItem value="clarity-version">
    ...
    <AccordionContent>
      <div className="pb-4 pt-2">
        ...
        ```toml Clarinet.toml
        # ✅ Good - all contracts use same version
        [contracts.token]
        clarity_version = 3
        epoch = 3.1
  ...
  <AccordionItem value="test-isolation">
    <AccordionTrigger>Tests interfering with each other</AccordionTrigger>
    <AccordionContent>
      <div className="pb-4 pt-2">
        If tests are affecting each other's state, ensure proper isolation in your Vitest configuration:

        ```js vitest.config.js
        poolOptions: {
          forks: { 
            singleFork: true,  // Reuse process for efficiency
            isolate: true      // Reset state between tests
          }
        }
        ```

        This configuration maintains test performance while ensuring each test starts with a clean state.
      </div>
    </AccordionContent>
  </AccordionItem>
</Accordion>
...
- Create your first project with the [quickstart guide](/tools/clarinet/quickstart)
- Learn about [contract development](/tools/clarinet/project-development)
- Explore [testing strategies](/tools/clarinet/testing-validation)