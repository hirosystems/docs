# Create a random number using stacks-block-height
...
At its core, we're using <HoverLink href="hover:random" className="text-[var(--ch-7)]">read-rnd</HoverLink> to fetch information about a specific block. This function is particularly looking for something called the <InlineCode codeblock={{language: "clarity", value: "id-header-hash"}}>id-header-hash</InlineCode>, which is essentially a unique identifier for the block.
