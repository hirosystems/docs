...
function CustomTable({ className, ...props }: TableProps) {
  ...
  const thead = tableContent.find(
    (child): child is React.ReactElement => React.isValidElement(child) && child.type === 'thead',
  );
  const tbody = tableContent.find(
    (child): child is React.ReactElement => React.isValidElement(child) && child.type === 'tbody',
  );

  const headerRows = thead
    ? React.Children.toArray((thead as React.ReactElement<{ children?: ReactNode }>).props.children)
    : [];
  const bodyRows = tbody
    ? React.Children.toArray((tbody as React.ReactElement<{ children?: ReactNode }>).props.children)
    : [];
  const rows = [...headerRows, ...bodyRows].filter((row): row is React.ReactElement =>
  ...
  const headers = React.Children.toArray(
    ...
    React.isValidElement(cell)
      ? ((cell as React.ReactElement<{ children?: ReactNode }>).props.children ?? '')
      : cell,
  );
