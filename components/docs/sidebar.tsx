  ...
  "relative flex flex-row items-center gap-2 rounded-md p-2 text-start text-fd-muted-foreground [overflow-wrap:anywhere] md:py-1.5 [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      active: {
        ...
        false: "transition-colors hover:text-primary hover:transition-none",
      },
    },
  }
);
  ...
  return (
    <Context.Provider value={context}>
      <Base.SidebarList
        ...
        {...props}
        className={cn(
          "fixed top-[calc(var(--fd-banner-height)+var(--fd-nav-height))] z-30 bg-background text-sm md:sticky md:h-(--fd-sidebar-height)",
          "max-md:inset-x-0 max-md:bottom-0 max-md:bg-background max-md:text-[15px] max-md:backdrop-blur-lg max-md:data-[open=false]:invisible",
          props.className
        )}
        style={
          {
      ...
      >
        <div
          {...inner}
          className={cn(
            "flex size-full max-w-full flex-col md:ms-auto md:w-(--fd-sidebar-width)",
            inner?.className
          )}
        >
          {props.children}
...
export function SidebarSeparator(props: HTMLAttributes<HTMLParagraphElement>) {
  ...
  return (
    <p
      {...props}
      className={cn(
        "gap-2 mb-2 px-2 mt-8 text-md text-primary font-fono font-medium [&_svg]:size-4 [&_svg]:shrink-0",
        props.className
      )}
      style={{
