...
function generateLLMsContent(pages: PageMetadata[], title: string, currentSection: string[] = []): string {
  ...
  for (const page of pages) {
    ...
    if (!groupedBySections.has(sectionKey)) {
      groupedBySections.set(sectionKey, []);
    }
    groupedBySections.get(sectionKey)!.push(page);
  }
  ...
  const needsSubsectionHeaders = sortedSections.length > 2 || (currentSection.length === 0 && sortedSections.length > 1);
...
async function generateAllLLMsTxt() {
  ...
  for (const page of allPages) {
    ...
    for (let i = 1; i <= page.section.length; i++) {
      ...
      sections.get(sectionPath)!.push(page);
    }
  }
  ...
  for (const [sectionPath, sectionPages] of sections) {
    ...
    console.log(`✅ Generated /${sectionPath}/llms.txt`);
  }
