...
(contract-call? .keys is-keyholder tx-sender) ;; true
```
...
If you are planning to experiment with your contract on `Devnet`, make sure to run `clarinet integrate` to deploy your contract locally. You can then use the `Devnet` network when calling your contract in the [Leather wallet](https://leather.io/install-extension).
...
**Message Signature**: To further enhance the security and authenticity of your application, implement a message signature feature. This feature should allow users to sign their messages using the private key of their authenticated wallet.
