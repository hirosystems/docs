...
In the previous hacks, you built smart contracts that have a lot of complex data living on-chain. And building a robust UI/UX around this data can be challenging due to the nature of blockchains. For instance, with our [Build a Decentralized Grants Program](./build-a-decentralized-grants-program.mdx) hack, you would need a simpler way to *display new grant proposals*, *notify users with updates to existing grants*, and more.

This is where [Chainhook](../chainhook) comes in. Chainhook is a powerful tool that allows you to trigger actions in response to on-chain events on Stacks and Bitcoin.
...
> _**Note**_
>
> You're welcome to use any Clarity app for your submission. The goal is to show you how to use Chainhook to enhance the functionality and user experience of your decentralized applications.
...
To illustrate how we can handle this event data, here is an example:
...
In this example, the server listens for POST requests at the `/api/events` route. When a request is received, in this case our Chainhook that we will set up in the next section, we can extract the event data from the request body and processes it.
...
- `transaction.metadata.receipt`: This attribute contains information about any events that were triggered from the function. It's a crucial part of the transaction metadata.
...
Here's what your POST route might look like after filtering for the relevant data:
...
For the next section, you will need to expose your local server via `https` so that Chainhook can deliver the `payload`. To do that, can run the following command:
...
Here's an example of how you might define this predicate for your contract:
...
> _**Note**_
>
> Make sure to swap out the details above to match your contract ABI
...
**Create more than one Chainhook**: Don't stop at just one! Follow the process from above to create more than one Chainhook for your dApp and parse the data accordingly.
...
<div>
  {isPast(new Date('2023-12-13')) ? (
    ...
    <div>
      ...
      <p>The deadline to submit is Wednesday, December 13th at midnight ET.</p>
