...
through a corporate account. This account should have permission to create resources.
...
Use a link in the table to launch the CloudFormation template in the AWS region if you wish to deploy a Gaia hub.
...
1. Launch the template using the preceding table's **Launch stack** button.
...
4. Specify configuration details for your Gaia hub:
   ...
   to enable SSH access to the EC2 instance. You should download this pair's `.pem` key file from the EC2 console.
   ...
   You can update this field if you wish to restrict SSH access to the EC2 instance to a certain IP.
...
7. Click **Next**.
8. Review your Gaia hub's configuration and make any necessary changes.
...
Your stack can take several minutes to launch. You can monitor your hub's **Events** tab to review the launch's current progress. When the launch is complete, the **Outputs** tab displays information about the hub. Select
...
It can take up to 15 minutes for your DNS record changes to propagate. Use the command
`dig A +short <yourdomain.co>` in a terminal to review if the changes have propagated. If the output of this command is the container
...
To SSH into your Gaia hub EC2 host directly, you must have the key file used in container creation. Access the host with
...
If you want to change your Gaia hub, there are two options. You can delete the entire CloudFormation stack
...
On the **Tags** tab, you can click **Manage tags** to update the relevant key-value pairs for the instance. If you change these tags, click **Save**, then select **Reboot instance** from the **Instance state** drop-down. Changes to
